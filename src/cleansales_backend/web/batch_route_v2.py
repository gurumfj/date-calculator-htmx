...
base_layout = lambda x: (
    Title("Cleansales"),
    Body(x, cls="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6"),
)
breeds = [
    ("黑羽", "黑羽", "batches/黑羽"),
    ("古早", "古早", "batches/古早"),
...
breeds_nav = lambda title, value, herf: Li(
    Button(
        ...
        {"@click": f"activeBreed = '{value}'"},
        {
            ...
            ":hx-vals": "JSON.stringify({start_date: document.getElementById('start_date').value})",
        },
        hx_get=herf,
...
def index():
    ...
    # 合併導航區域，包含品種選擇和日期選擇
    nav_section = Nav(
        Hidden(id="activeBreed", name="breed", **{":value": "activeBreed"}),
        # 品種選擇 - 總是可見
        Header(
            Div(
                H4("選擇品種", cls="text-sm font-medium text-gray-700 mb-3 text-center"),
                Ul(
                    *[breeds_nav(title, value, herf) for title, value, herf in breeds],
            ...
            Div(
                Button(
                    ...
                    Span(
                        ...
                        cls="transform transition-transform duration-200 inline-block",
                    ),
        ...
        Section(
            Div(
                ...
                Div(
                    Input(
                        {":hx-vals": "JSON.stringify({breed: activeBreed})"},
                        id="start_date",
                        name="start_date",
                        type="date",
                        value=today,
                        hx_get="batches/query",
                        hx_include="this",
                        hx_target="#load_batches",
                        hx_swap="innerHTML transition:true",
                        hx_trigger="change",
    ...
    load_batches = Div(
        ...
        hx_trigger="load",
        hx_swap="innerHTML transition:true",
        cls="container mx-auto transition-opacity duration-300",
    )

    # 將所有元件包在一個 Alpine.js 容器中
    app_container = Div(header, nav_section, load_batches, {"x-data": "{ activeBreed: '黑羽', datePickerOpen: false }"})

    return base_layout(app_container)


@rt("/batches/query")
def query_batches_by_params(breed: str | None = None, start_date: str | None = None, direction: str | None = None):
    """通過參數查詢批次資料的路由"""
    if breed is None:
        breed = "黑羽"
    return query_batches_route(breed, start_date, direction)


@rt("/batches/{breed}")
def query_batches_route(breed: str, start_date: str | None = None, direction: str | None = None):
    breed = breed.strip()
    if breed not in ["黑羽", "古早", "舍黑", "閹雞"]:
        breed = "黑羽"
    ...
    # 根據方向計算查詢範圍
    if direction == "before":
        # 查詢更早的批次 (before_start_date)
        query_start_date = datetime.strptime(start_date, "%Y-%m-%d").date() - timedelta(days=30)
    elif direction == "after":
        # 查詢更新的批次 (after_start_date)
        query_start_date = datetime.strptime(start_date, "%Y-%m-%d").date() + timedelta(days=30)
    else:
        # 預設查詢 (30天前到今天)
        query_start_date = datetime.strptime(start_date, "%Y-%m-%d").date()

    batches_data, has_earlier, has_later = query_batches_data(breed, query_start_date)

    # 使用UI組件函式渲染按鈕
    load_later_btn = _render_load_button(breed, query_start_date, has_later, "later")
    load_earlier_btn = _render_load_button(breed, query_start_date, has_earlier, "earlier")

    batches_container = Div(
        load_earlier_btn,
        Hidden(query_start_date.strftime("%Y-%m-%d"), id="start_date"),
        *[_render_batch(dict(row)) for row in batches_data][::-1],
        load_later_btn,
        id="batches_container",
        cls="grid gap-4",
    )

    return batches_container


def query_batches_data(breed: str, start_date: date):
    stmt = """
    SELECT 
    ...
    """
    start_date_params = (breed, start_date, start_date)
    earlier_date_params = (breed, start_date - timedelta(days=30), start_date - timedelta(days=30))
    later_date_params = (breed, start_date + timedelta(days=30), start_date + timedelta(days=30))
    sql_service = SQLiteDataService(DB_PATH)
    conn = sql_service.get_db_connection()
    cursor = conn.cursor()
    cursor.execute(stmt, start_date_params)
    data = cursor.fetchall()
    cursor.execute(stmt, earlier_date_params)
    has_earlier = True if cursor.fetchone() else False
    cursor.execute(stmt, later_date_params)
    has_later = True if cursor.fetchone() else False
    return data, has_earlier, has_later


def _render_batch(data):
    percentage = round(data["percentage"], 1)
    percentage_color = (
        "text-green-600" if percentage >= 80 else "text-yellow-600" if percentage >= 50 else "text-red-600"
    )
    ...
    return Article(
        ...
        Header(
            Div(
                ...
                Span(
                    data["chicken_breed"],
                    cls="inline-block bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium",
                ),
                ...
                *[
                ...
                Div(
                    ...
                    Span(
                        ...
                        cls="text-gray-400 transform transition-transform duration-200 inline-block",
                    ),
...
def _render_load_button(breed: str, query_start_date: date, has_data: bool, button_type: str):
    """渲染載入資料按鈕

    Args:
        breed: 雞種
        query_start_date: 查詢起始日期
        has_data: 是否有資料可載入
        button_type: 按鈕類型 ('later' 或 'earlier')
    """
    if not has_data:
        return Div()

    if button_type == "later":
        button_text = f"載入更新 {(query_start_date + timedelta(days=30)).strftime('%Y-%m-%d')}"
        button_id = "load_latest"
        direction = "after"
        button_cls = "w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 mb-4"
    else:  # earlier
        button_text = f"載入更早 {(query_start_date - timedelta(days=30)).strftime('%Y-%m-%d')}"
        button_id = "load_earlier"
        direction = "before"
        button_cls = "w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 mt-4"
    ...
    return Div(
        Button(
            ...
            hx_vals=json.dumps({"start_date": query_start_date.strftime("%Y-%m-%d"), "direction": direction}),
            hx_target="#batches_container",
