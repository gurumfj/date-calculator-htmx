...
class DataProcessor:
    def __init__(self, db_path: str):
        self.db_path = db_path
        self.validators = {
            "breed": (BreedRecordValidator, "breed", [
                "unique_id", "farm_name", "address", "farm_license", "farmer_name",
                "farmer_address", "batch_name", "sub_location", "veterinarian",
                "is_completed", "chicken_breed", "breed_date", "supplier",
                "breed_male", "breed_female", "event_id", "created_at"
            ]),
            "sale": (SaleRecordValidator, "sale", [
                "unique_id", "is_completed", "handler", "sale_date", "batch_name",
                "customer", "male_count", "female_count", "total_weight", "total_price",
                "male_price", "female_price", "unpaid", "event_id", "created_at"
            ]),
            "feed": (FeedRecordValidator, "feed", [
    ...
    def sync_database(
        self, file_type: str, records: List[Dict[str, Any]]
        ...
        try:
            ...
            if ids_to_delete:
                ...
                conn.execute(
                    f"DELETE FROM {table_name} WHERE unique_id IN ({placeholders})",
                    list(ids_to_delete)
                )
...
class UploadCommandHandler:
    ...
    async def handle(self, command: UploadFileCommand) -> Dict[str, Any]:
        ...
        try:
            ...
            valid_records, invalid_count = self.data_processor.validate_and_process_data(
                df, file_type, event_id
            )
            ...
            inserted_count, deleted_count, duplicate_count, duplicates_removed = (
                self.data_processor.sync_database(file_type, valid_records)
            )
            ...
            final_message = (
                f"處理 {file_type} 資料：" + "，".join(message_parts) 
                if message_parts else f"處理 {file_type} 資料完成"
            )
