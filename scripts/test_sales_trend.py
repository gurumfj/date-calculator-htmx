...
def analyze_batch_sales(batch_name: str = None):
    ...
    try:
        with db.get_session() as session:
            ...
            if batch_name:
                # 查詢特定批次
                batches = query_service.get_breed_by_batch_name(session, batch_name)
            else:
                # 查詢所有未結案的批次
                batches = query_service.get_breeds_is_not_completed(session)
        ...
        for batch in batches:
            ...
            total_count = daily_df["male_count"].sum() + daily_df["female_count"].sum()
